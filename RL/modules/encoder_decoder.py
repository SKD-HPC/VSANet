...
class Transformer(nn.Module):
    ...
    def forward(self, src, symptom_feature, tgt, src_mask, tgt_mask):
        sem_mask = None
        return self.decode(self.encode(src, src_mask), src_mask, self.encoder(symptom_feature, sem_mask), tgt, tgt_mask)

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, hidden_states, src_mask, symptom_feature, tgt, tgt_mask):
        memory = self.rm.init_memory(hidden_states.size(0)).to(hidden_states)
        memory = self.rm(self.tgt_embed(tgt), memory)
        return self.decoder(self.tgt_embed(tgt), symptom_feature, hidden_states, src_mask, tgt_mask, memory)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, symptom_feature, hidden_states, src_mask, tgt_mask, memory):
        for layer in self.layers:
            x = layer(x, symptom_feature, hidden_states, src_mask, tgt_mask, memory)
        return self.norm(x)


class DecoderLayer(nn.Module):
    def __init__(self, d_model, self_attn, src_attn, sem_attn, feed_forward, dropout, rm_num_slots, rm_d_model):
        super(DecoderLayer, self).__init__()
        self.d_model = d_model
        self.self_attn = self_attn
        self.src_attn = src_attn
        self.feed_forward = feed_forward
        self.sublayer = clones(ConditionalSublayerConnection(d_model, dropout, rm_num_slots, rm_d_model), 4)
    ...
    def forward(self, x, symptom_feature, hidden_states, src_mask, tgt_mask, memory):
        ...
        #src_attn
        v = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask), memory)
        #sem_attn
        s = self.sublayer[2](x, lambda x: self.src_attn(x, n, n), memory)

        # AOA module
        V = torch.sigmoid(self.W1(v) + self.W2(x)) * (self.W3(v) + self.W4(x))
        S = torch.sigmoid(self.W5(s) + self.W6(x)) * (self.W7(s) + self.W8(x))
        # Score-based Fusion Module

        alpha = torch.sigmoid(self.W9(V) + self.W10(S) + self.W11(x))
        x = alpha * V + (torch.ones(alpha.shape).cuda() - alpha) * S

        return self.sublayer[3](x, self.feed_forward, memory)
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        model = Transformer(
            ...
            Decoder(
                DecoderLayer(self.d_model, c(attn), c(attn), c(attn), c(ff), self.dropout, self.rm_num_slots, self.rm_d_model),
                self.num_layers),
            lambda x: x,
    ...
    def _forward(self, fc_feats, att_feats, symptom_feature, seq, att_masks=None):

        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        out = self.model(att_feats, symptom_feature, seq, att_masks, seq_mask)
        outputs = F.log_softmax(self.logit(out), dim=-1)
        return outputs

    def core(self, it, symptom_feature, fc_feats_ph, att_feats_ph, memory, state, mask):

        if len(state) == 0:
            ys = it.unsqueeze(1)
        else:
            ys = torch.cat([state[0][0], it.unsqueeze(1)], dim=1)
        out = self.model.decode(memory, mask, symptom_feature, ys, subsequent_mask(ys.size(1)).to(memory.device))
