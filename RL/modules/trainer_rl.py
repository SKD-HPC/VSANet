...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, symptom_feature) in enumerate(self.train_dataloader):
            images, reports_ids, reports_masks, symptom_feature = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device), symptom_feature.to(self.device)
            init_scorer()
            self.model.eval()
            with torch.no_grad():
                greedy_res, _ = self.model(images, symptom_feature, mode='sample',
                                           update_opts={'sample_method': self.args.sc_sample_method,
                                                        'beam_size': self.args.sc_beam_size})
            self.model.train()
            gen_result, sample_logprobs = self.model(images, symptom_feature, mode='sample',
            ...
            output = self.model(images, symptom_feature, reports_ids, mode='train')
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, symptom_feature) in enumerate(self.val_dataloader):
                images, reports_ids, reports_masks, symptom_feature = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device), symptom_feature.to(self.device)
                output, _ = self.model(images, symptom_feature, mode='sample')
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks, symptom_feature) in enumerate(self.test_dataloader):
                images, reports_ids, reports_masks, symptom_feature = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device), symptom_feature.to(self.device)
                output, _ = self.model(images, symptom_feature, mode='sample')
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
