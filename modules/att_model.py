...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, symptom_feature, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, symptom_feature, fc_feats, att_feats, p_att_feats, state, att_masks)
    ...
    def _sample_beam(self, symptom_feature, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        logprobs, state = self.get_logprobs_state(it, symptom_feature, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
        ...
        self.done_beams = self.beam_search(state, symptom_feature, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, symptom_feature, att_masks=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(symptom_feature, fc_feats, att_feats, att_masks, opt)
        if group_size > 1:
        ...
        for tt in range(self.max_seq_length + group_size):
            for divm in range(group_size):
                ...
                if t >= 0 and t <= self.max_seq_length - 1:
                    ...
                    logprobs, state_table[divm] = self.get_logprobs_state(it, symptom_feature, p_fc_feats, p_att_feats, pp_att_feats,
